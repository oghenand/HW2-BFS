# Assignment 2
Breadth-first search is a graph algorithm that traverses through a graph. Given a start (or source) node, the algorithm explores "level by level": each neighboring node (child) is visited. Thereafter, the next depth is explored, with each child of each child explored. This process iterates until all nodes reachable from the start node are visited. In an unconnected graph, only the nodes reachable from the start node are processed by the algorithm. Because BFS explores nodes depth-by-depth, it will encounter a given node that requires the minimum depth/number of layers to reach. Due to this, it will only find longer paths after it first reaches the given node, meaning that it finds the shortest path from a start node to a desired end node, given that the end node exists in the graph and is connected/is a descendant of the start node. 

My implementation of BFS uses the provided networkx package to load and work with the graph data. Other ways to store data for BFS are an adjacency matrix/dictionary (where for a given node, all the nodes to which it has edges are listed). BFS uses a first-in-first-out (FIFO) queue system to process nodes and ensure that the closest nodes are visited first. I implement this with the deque function in collections from python. Starting with the start node, the BFS code checks for edge cases (i.e. making sure the start node is in the graph), and then uses a while loop to traverse the graph. This loop finishes when (1) the target/end node is reached or (2) when all nodes connected to the start node are visited, at which point the traversal order is returned (if the end node is None) or None is returned to denote to path from the start to end when an end node is given.

The BFS code is tested for correctness mainly through comparing its results to results from the built-in networkx traversal and shortest paths functions. It also checks for test cases/errors (i.e. not providing a start node).
